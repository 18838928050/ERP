...
<html lang="zh-CN">
	...
	<body>
		...
		<%-- 	<td>
			<s:if test='purchaseBillExtVOs[#st.index].examined=="Y"'><s:a action="tuihuobiaofanshenhe_chakan?purchaseBillVO.id=%{id}">已审核</s:a></s:if>
			<s:if test='purchaseBillExtVOs[#st.index].examined=="N"'><s:a action="tuihuobiaoshenhe_chakan?purchaseBillVO.id=%{id}">未审核</s:a></s:if>
			<s:if test='purchaseBillExtVOs[#st.index].stoc=="N"'><s:a action="tuihuodanfanchuku_fanchuku?purchaseBillVO.id=%{id}">已出库</s:a></s:if>
			<s:if test='purchaseBillExtVOs[#st.index].stoc=="Y"'><s:a action="tuihuodanchuku_chuku?purchaseBillVO.id=%{id}">未出库</s:a></s:if>
			<s:if test='cost-purchaseBillExtVOs[#st.index].paiedCost==0'>已结清</s:if>
			<s:if test='cost-purchaseBillExtVOs[#st.index].paiedCost!=0'><s:a action="tuihuobiaoshenhe_chakan?purchaseBillVO.id=%{id}">未结清</s:a></s:if>
			</td> --%>
			<td>
			
			<s:if test='purchaseBillExtVOs[#st.index].examined=="Y"'><s:a href="javascript:showWin(['900px', '500px'],'tuihuobiaofanshenhe_chakan?purchaseBillVO.id=%{id}');" >已审核</s:a></s:if>
			<s:if test='purchaseBillExtVOs[#st.index].examined=="N"'><s:a href="javascript:showWin(['900px', '500px'],'tuihuobiaoshenhe_chakan?purchaseBillVO.id=%{id}');" >未审核</s:a></s:if>
			<s:if test='purchaseBillExtVOs[#st.index].stoc=="N"'>    <s:a href="javascript:showWin(['900px', '500px'],'tuihuodanfanchuku_fanchuku?purchaseBillVO.id=%{id}');" >已出库</s:a></s:if>
			<s:if test='purchaseBillExtVOs[#st.index].stoc=="Y"'>    <s:a href="javascript:showWin(['900px', '500px'],'tuihuodanchuku_chuku?purchaseBillVO.id=%{id}');" >未出库</s:a></s:if>
			<s:if test='cost-purchaseBillExtVOs[#st.index].paiedCost==0'>已结清</s:if>
			<s:if test='cost-purchaseBillExtVOs[#st.index].paiedCost!=0'><s:a action="#">未结清</s:a></s:if>

...
<body oncontextmenu="return false" onload="doInit();" class="sheet">
				...
				<div class="table-list">
					<table width="100%">
						<tbody>
							...
							<s:iterator value="feeBills" var="feeBill" status="st">
								<tr class="query_list_data_trSelected" index="1"
									...
									<td>${ billCode } <input type="radio"
										name="query_checkbox" value="171" style="display: none">
									</td>
									<td><s:date name="feeDate" format="yyyy-MM-dd" /></td>
									<td>${receiverName }</td>
									<td>${cost }</td>
									<td>${mark }</td>
									<td>${managerName }</td>
									...
									<s:elseif test=' #feeBill.examined.equals("N") '>
										<td>未审核</td>
									</s:elseif>
					...
					<div class="table-bot clearfix">
						<div class="pull-right">
							...
							页 <a style="cursor: pointer" onclick=""><img
	...
	function doShow(id){
		var url="Accounts_toFeeBill?id="+id;
	...
	function doDisabled(id){
		var url="Accounts_toDisabledFeeBill";
		ShowIframe(['455px','235px'], url, id, function callBack(){
					doDisabledCallback(id);
				},'费用单作废');
	}
	function doDisabledCallback(id){
		if(popRetVal!=null){
			$.post("Accounts_disabledFeeBill",{id:id,reason:popRetVal,show_aler:"Y"},function(data,status){

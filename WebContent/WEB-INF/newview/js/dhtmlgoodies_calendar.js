...
var languageCode = 'ch';	// Possible values: 	en,ge,no,nl,es,pt-br,fr
...
// [year] = Current year
var todayStringFormat = '[year]. [monthString]. [day] [UCFdayString] [todayString]';
var pathToImages = '/erp-demo/images/';	// Relative to your HTML file
...
function displayCalendar(inputField,format,buttonObj,displayTime,okCallBack)
{

	//输入框输入的日期格式与format不符的话,下面的代码转换有问题
	//目前系统都使用yyyy-mm-dd,先临时解决
	if(format=="yyyy-mm-dd"){
		var tmpArr=inputField.value.split("-");
		var inputFieldVal=""
		if(tmpArr.length==3){
			if(tmpArr[0].length==4 && !isNaN(tmpArr[0]) && !isNaN(tmpArr[1]) && !isNaN(tmpArr[2])){
				try{
					var d=new Date(tmpArr[0],tmpArr[1]-1,tmpArr[2]);
					var inputFieldValMon;
					var inputFieldValDat;
					if(d.getMonth()<9){//月要+1
						inputFieldValMon="0"+(d.getMonth()+1);
					}else{
						inputFieldValMon=d.getMonth()+1;
					}
					if(d.getDate()<10){
						inputFieldValDat="0"+d.getDate();
					}else{
						inputFieldValDat=d.getDate();
					}
					inputFieldVal=d.getFullYear()+"-"+inputFieldValMon+"-"+inputFieldValDat;
				}catch(ex){}
			}
		}
